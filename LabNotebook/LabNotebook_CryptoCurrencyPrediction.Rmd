...
To provide us with the opportunity to predict smaller time frames, we used the [Crypto Compare API](https://min-api.cryptocompare.com/) to obtain historical prices for each of our 5 coins. Using UTC/GMT time, we limited our search to prices from January 01, 2016 (midnight UTC) to February 28, 2018 (23:00 UTC). The code below is what we used for our analysis. In the code, the printout line before the save was used to verify that we indeed captured data on the desired first and last time points. 
```{r message=FALSE, warning=FALSE, eval=FALSE}
...
```
Our initial results from running the code above is stored at: [HourlyPrices-28-2-2018.RData](https://github.com/fmegahed/cryptopredictions/blob/master/Code/DataCollection/Data/HourlyPrices-28-2-2018.RData) (which can be accessed by clicking the link on the hyperlink.) Note that we will need to clean this data prior to conducting any analysis; for example, BCH was forked on August 1, 2017 and this is why the API returned zeros for all the variables (with the exception of the time stamp) before that date. See [Screen Capture of the BCH data (Output using the base R str function)](https://github.com/fmegahed/cryptopredictions/blob/master/LabNotebook/Figures/Example.how.API.handling.BCH.before.Release.Date.JPG). **This will be addressed in the data cleaning/modeling.**  
...
save(release.ethereum,total.scraped.and.manually.counted,totalexpected, file = "Data/EthereumGitHub-26-2-2018.RData")
```
